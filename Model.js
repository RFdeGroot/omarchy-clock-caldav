...
// ---- Calendar helpers, added for the CalDAV integration. Still Qt-free, so
//      the same node tests cover them.

// The date_path shape bin/meetings-widget emits for a day: "y/m/d", no leading
// zeroes, the way Google Calendar wants it in a URL. Used to line the event
// payload up with the grid Model.monthGrid draws.
function datePath(year, month, day) {
  return year + "/" + (Number(month) + 1) + "/" + day
}

function datePathForDate(date) {
  return datePath(date.getFullYear(), date.getMonth(), date.getDate())
}

// Index a flat list of day entries (a week or month payload's `days`) by their
// date_path, so a grid cell can look its events up in one hop.
function indexByDatePath(days) {
  var map = {}
  for (var i = 0; i < (days ? days.length : 0); i++)
    map[String(days[i].date_path)] = days[i]
  return map
}

// Up to `max` distinct calendar colours present on a day entry, all-day events
// first. `{ colors: [...], more: bool }` — `more` drives the faint "+" the grid
// shows when a day has more calendars than dots fit.
function dayColors(dayEntry, max) {
  var cap = max || 3
  var seen = {}
  var out = []
  function take(list) {
    for (var i = 0; i < (list ? list.length : 0); i++) {
      var c = String((list[i] && list[i].color) || "")
      if (c === "" || seen[c]) continue
      seen[c] = true
      out.push(c)
    }
  }
  if (dayEntry) { take(dayEntry.allday); take(dayEntry.events) }
  return { colors: out.slice(0, cap), more: out.length > cap }
}

// Every event on a day entry, all-day first, then the timed ones in the order
// the script already sorted them.
function dayItems(dayEntry) {
  if (!dayEntry) return []
  var out = []
  var allday = dayEntry.allday || []
  var events = dayEntry.events || []
  for (var a = 0; a < allday.length; a++) out.push(allday[a])
  for (var t = 0; t < events.length; t++) out.push(events[t])
  return out
}

// Whole months from (aYear,aMonth) to (bYear,bMonth): the offset bin/meetings-
// widget's `month` command counts in.
function monthDelta(aYear, aMonth, bYear, bMonth) {
  return (bYear * 12 + Number(bMonth)) - (aYear * 12 + Number(aMonth))
}

// Whole weeks between the Monday of `from` and the Monday of `date`: the offset
// the `week` command counts in.
function weekDelta(fromDate, date) {
  function monday(d) {
    var m = new Date(d.getFullYear(), d.getMonth(), d.getDate())
    m.setDate(m.getDate() - ((m.getDay() + 6) % 7))
    return m
  }
  return Math.round((monday(date).getTime() - monday(fromDate).getTime()) / 604800000)
}

// Keep a selected day-of-month in range when the month changes under it: the
// 31st becomes the 28th stepping into February, then the 31st again on the way
// back out is the caller's job (it keeps the desired day separately).
function clampDay(year, month, desiredDay) {
  var last = new Date(year, Number(month) + 1, 0).getDate()
  return Math.min(Math.max(1, Number(desiredDay)), last)
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    datePath: datePath,
    datePathForDate: datePathForDate,
    indexByDatePath: indexByDatePath,
    dayColors: dayColors,
    dayItems: dayItems,
